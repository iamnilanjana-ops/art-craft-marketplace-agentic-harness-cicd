...
The storage MCP allow-list was changed so that the `documentation-writer` role temporarily lost its previously granted `read\_entry` permission.
...
The `documentation-writer` role was removed from the `read\_entry` allow-list.
...
The `documentation-writer` role was restored to the `read\_entry` allow-list.
